        ...
        <div className="space-y-0 border-t border-[var(--color-accent-line)]/30">
          {SERVICES.map((service) => (
            <Link 
              ...
              className="group flex flex-col md:flex-row items-baseline md:items-center justify-between py-12 border-b border-[var(--color-accent-line)]/30 hover:bg-[var(--color-bg-sec)] hover:px-8 hover:-mx-8 hover:text-[var(--color-text-sec)] transition-all duration-500 ease-[cubic-bezier(0.25,0.46,0.45,0.94)]"
            >
              <div className="flex items-baseline space-x-8 md:space-x-16">
                 <span className="font-mono text-sm text-[var(--color-accent-line)] group-hover:text-[var(--color-text-sec)]/60 transition-colors">
                    {service.number}
                 </span>
                 <h2 className="font-serif text-3xl md:text-4xl group-hover:translate-x-4 transition-transform duration-500">
                    {service.title}
                 </h2>
              </div>
              
              <div className="mt-6 md:mt-0 flex items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">
                  <span className="text-xs tracking-widest uppercase mr-4">View Details</span>
                  <ArrowRight size={20} />
              </div>
            </Link>
          ))}
